...
<head>
    ...
    <title>Quarto - Ocean Blue Hotel</title>
        ...
        <div class="overflow-x-auto mx-auto">
            <table class="min-w-full bg-white border border-gray-300">
                ...
                <tbody>
                    ...
                    $query = "SELECT * FROM quarto ORDER BY numero";
                    $result = mysqli_query($conn, $query);

                    if (mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            echo "<tr>";
                            echo "<td class='py-2 px-4 border-b'>" . $row["numero"] . "</td>";
                            echo "<td class='py-2 px-4 border-b'>" . $row["nome"] . "</td>";
                            echo "<td class='py-2 px-4 border-b'>$" . number_format($row["preco"], 2) . "</td>";
                            echo "<td class='py-2 px-4 border-b'>
                                    <a href='alterar_quarto.php?numero=" . $row["numero"] . "' class='text-blue-500 mr-2'>Alterar</a>
                                    <a href='excluir_quarto.php?numero=" . $row["numero"] . "' class='text-red-500'>Excluir</a>
                                  </td>";
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='4' class='py-2 px-4 border-b'>Nenhum quarto foi encontrado.</td></tr>";
                    }

                    echo "<tr><td colspan='8' class='py-2 px-4 '><a href='adicionar_quarto.php' class='text-green-500'>Adicionar Quarto</a></td></tr>";
